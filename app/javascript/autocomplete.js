...
document.addEventListener("turbo:load", function () {
  ...
  if (gon.addresses) {
    ...
    $("#ride_start_address_attributes_name")
      .off("autocompleteselect")
      .on("autocompleteselect", function (event, ui) {
        document.getElementById("ride_start_address_attributes_street").value =
          ui.item.street;
    ...
    $("#ride_start_address_attributes_street")
      .off("autocompleteselect")
      .on("autocompleteselect", function (event, ui) {
    ...
    $(document)
      .off("focus", ".dest-autocomplete")
      .on("focus", ".dest-autocomplete", function () {
        const $input = $(this);
  ...
  if (gon.duplicate_info) {
    ...
    if (gon.passengers && info.passenger_id) {
      const passengerObj = gon.passengers.find(
        (p) => p.id === info.passenger_id,
      );
      ...
      if (passengerObj) {
        ...
        // B. Trigger existing autocomplete-select logic so all passenger fields fill in
        passengerInput.trigger("autocompleteselect", { item: passengerObj });

        // C. Overwrite Start Address with the duplicated ride's start address
        if (info.start_address) {
          const addr = info.start_address;
          if (addr.name) {
            $("#ride_start_address_attributes_name").val(addr.name);
          }
          if (addr.street) {
            $("#ride_start_address_attributes_street").val(addr.street);
          }
          if (addr.city) {
            $("#ride_start_address_attributes_city").val(addr.city);
          }
          if (addr.phone) {
            $("#ride_start_address_attributes_phone").val(addr.phone);
          }
        }
      }
    }
